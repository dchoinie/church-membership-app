...
export default function Forgot2FAPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch("/api/2fa-reset/request", {
        method: "POST",
      ...
      if (!response.ok) {
        throw new Error(data.error || "Failed to send 2FA reset email");
      }

      setSuccess(true);
    } catch (err) {
      setError(err instanceof Error ? err.message : "Failed to send 2FA reset email");
    } finally {
      setIsSubmitting(false);
    }
  };
        ...
        <div className="relative z-10 max-w-lg">
          <div className="mb-8">
            ...
            <h1 className="text-5xl font-bold tracking-tight mb-4">
              Reset Your{" "}
              <span className="bg-gradient-to-r from-primary to-accent bg-clip-text text-transparent">Two-Factor Authentication</span>
            </h1>
            <p className="text-xl text-muted-foreground mb-8">
              Enter your email address and we&apos;ll send you a link to reset your 2FA. You&apos;ll then need to set it up again.
            </p>
          </div>
        </div>
      </div>
          ...
          <Card className="border-2 shadow-lg">
            <CardHeader className="space-y-1 text-center">
              ...
              <CardDescription className="text-base">
                Enter your email address to receive a 2FA reset link
              </CardDescription>
            </CardHeader>
            <CardContent>
              {success ? (
                <div className="space-y-4">
                  <div className="rounded-md bg-green-50 dark:bg-green-900/20 p-4 text-sm text-green-800 dark:text-green-200 border border-green-200 dark:border-green-800">
                    ...
                    <p>
                      If an account with that email has 2FA enabled, we&apos;ve sent you a reset link.
                      Please check your inbox and follow the instructions to reset your 2FA, then sign in to set it up again.
                    </p>
